...
variable {G k V W : Type u} [Monoid G] [Field k] [AddCommGroup V] [Module k V] [AddCommGroup W]
...
theorem bijective_or_eq_zero [IsIrreducible ρ] [IsIrreducible σ] (f : IntertwiningMap ρ σ) :
  ...
  · right
    ...
    ext x
    dsimp
    rw [show ((IntertwiningMap.toLinearMap 0) : V →ₗ[k] W) = 0 from rfl, LinearMap.zero_apply]
    rw [show f.toLinearMap x = (asLinearMap f) x from rfl, h, LinearMap.zero_apply]
...
theorem scalar_intertwining_map_of_irreducible_alg_closed [IsIrreducible ρ] [FiniteDimensional k V]
  ...
  · obtain ⟨c, hc⟩ := Module.End.exists_eigenvalue (f.toLinearMap)
    ...
    rw [show f 0 = f.toLinearMap 0 from rfl, f.toLinearMap.map_zero]
    rw [show f b = f.toLinearMap b from rfl, hb, add_neg_cancel]
